...
```bash
python3 /home/user/ken/orchestrator/memory_ops.py recall "session checkpoint" --domain romans --limit 5
```
...
```bash
python3 /home/user/ken/orchestrator/memory_ops.py encode romans decision \
...
```bash
python3 /home/user/ken/orchestrator/memory_ops.py recall "rate limit recovery" --domain romans --limit 3
```
...
## Romans-Specific Target Files

| Task type | Load only |
|---|---|
| Sermon draft | The specific sermon .md only |
| Sermon map update | .claude/sermon-map.md only |
| Theological map | .claude/theological-map.md only |
| Voice audit | The sermon being audited only |

## Do Not Break List

- sermon-map.md format integrity (all 8 formats)
- Scripture references verified (ESV)
- No fabricated quotes or illustrations
