...
- **In-scope**: All `.md` files under `sermons/` (ot, nt, topical), `services/`, `teaching/`
- **Out-of-scope**: `quotes-and-references/`, `sebts-reference/`
- Without scope boundaries, AUDIT would flag 1500+ reference files as "orphans"

### Procedure

1. **Scan sermon files** on the filesystem (Glob for `sermons/**/*.md`, `services/**/*.md`, `teaching/**/*.md`)
